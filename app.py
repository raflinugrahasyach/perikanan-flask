...
import joblib
from werkzeug.utils import secure_filename # Untuk upload file yang aman
import threading
...
try:
    ...
    # (Placeholder configs... )
    MODEL_PATHS = {'species_model': 'Model_Klasifikasi/keras_model1.h5', 'conservation_model': 'Model_Klasifikasi/keras_model2.h5', 'species_labels': 'Model_Klasifikasi/labels1.txt', 'conservation_labels': 'Model_Klasifikasi/labels2.txt'}
...
# --- Inisialisasi Firebase Admin SDK (Cerdas: HF Secret atau File Lokal) ---
try:
    if not firebase_admin._apps:
        # 1. Coba ambil dari Hugging Face "Secrets" (untuk production)
        firebase_secret_json = os.environ.get('FIREBASE_SERVICE_ACCOUNT_JSON')
        
        if firebase_secret_json:
            print("Mencoba inisialisasi Firebase dari HF SECRET...")
            # Ubah string JSON dari secret menjadi dictionary
            cred_dict = json.loads(firebase_secret_json)
            cred = credentials.Certificate(cred_dict)
            firebase_admin.initialize_app(cred, {
                'databaseURL': 'https://perikanan-73237-default-rtdb.firebaseio.com' # Pastikan URL ini benar
            })
            print("Koneksi Firebase Admin SDK dari SECRET berhasil.")
        
        # 2. Jika tidak ada secret, coba dari file lokal (untuk development)
        elif os.path.exists('firebase-service-account.json'):
            print("Mencoba inisialisasi Firebase dari FILE LOKAL...")
            cred = credentials.Certificate('firebase-service-account.json')
            firebase_admin.initialize_app(cred, {
                'databaseURL': 'https://perikanan-73237-default-rtdb.firebaseio.com'
            })
            print("Koneksi Firebase Admin SDK dari FILE LOKAL berhasil.")
        
        # 3. Jika keduanya gagal
        else:
            print("PERINGATAN: 'FIREBASE_SERVICE_ACCOUNT_JSON' secret tidak ditemukan dan 'firebase-service-account.json' tidak ada.")
            print("Fitur Peta tidak akan berfungsi.")
...
# --- Helper 4: Fungsi untuk /classification (dari marine_classification.py) ---
class MarineClassifier:
    ...
    def load_models_lazy(self):
        ...
        print("--- MEMUAT MODEL KLASIFIKASI (Lazy Load) ---")
        try:
            m1_path = MODEL_PATHS['species_model']; m2_path = MODEL_PATHS['conservation_model']
            l1_path = MODEL_PATHS['species_labels']; l2_path = MODEL_PATHS['conservation_labels']
            if not all(os.path.exists(p) for p in [m1_path, m2_path, l1_path, l2_path]):
                print(f"Warning: File model klasifikasi tidak ditemukan. Cek path: {m1_path}, {m2_path}, {l1_path}, {l2_path}"); return False
            ...
            self.model1 = tf.keras.models.load_model(m1_path, compile=False, custom_objects=custom_objects)
            self.model2 = tf.keras.models.load_model(m2_path, compile=False, custom_objects=custom_objects)
    ...
    def predict(self, image):
        ...
        try:
            pred1 = self.model1(processed_image, training=False) 
            pred2 = self.model2(processed_image, training=False)
        except Exception as e:
...
def get_classifier():
    ...
    with classifier_lock:
        if _global_classifier is None:
            print("--- Menginisialisasi Singleton Classifier ---")
...
def classification():
    ...
    # Jika method GET, tampilkan halaman
