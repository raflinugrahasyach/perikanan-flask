...
import numpy as np # Diperlukan untuk dummy class
...
try:
    ...
    class MarketForesight:
        def __init__(self): self.data = None; self.processed_data = None
        ...
        def generate_alerts(self, preds, threshold_percent, method='arima'): return pd.DataFrame()
...
def _plot_predictions(df, hist_data=None):
    ...
    if hist_data is not None and not hist_data.empty:
        hist_values_col = 'Harga_Per_Kg' if 'Harga_Per_Kg' in hist_data.columns else hist_data.columns[0]
        ax.plot(hist_data.index, hist_data[hist_values_col], 'b-', label='Data Historis', alpha=0.7)
...
def main():
    ...
    st.markdown('<div class="chart-container">', unsafe_allow_html=True)
    st.subheader("1. Data & Pengaturan")

    col1, col2 = st.columns(2)
    with col1:
        uploaded = st.file_uploader("Upload CSV atau Excel (opsional)", type=["csv", "xlsx", "xls"])
    ...
    with col2:
        if st.button("Buat Data Sampel (36 bulan)"):
            ...
            st.success("Data sampel dibuat dan dimuat ke session.")
